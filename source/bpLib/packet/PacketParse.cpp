...
std::string hex_out_s(T val, bool mac = false) {
    ...
    if (mac) {
        for (int i = 0; i < 6; i++) sformatter << std::setw(2) << 0u + ((uint8_t *)val)[i];
    } else {
...
uint8_t ip_header_t::ihl() const { return ver_ihl & 0x0F; }

size_t ip_header_t::size() const { return ihl() * sizeof(uint32_t); }
...
bp_proxy_t load(std::istream &stream, bool ntoh) {
    bp_proxy_t proxyInfo{};
    stream.read(reinterpret_cast<char *>(&proxyInfo), sizeof(proxyInfo));
    return proxyInfo;
}
